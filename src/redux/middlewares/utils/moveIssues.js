...
export function getNewLabels(card, label) {
  ...
  const labels = card.issue.labels.filter(
    _label =>
      UNCATEGORIZED_NAME !== _label.name && !KANBAN_LABEL.test(_label.name)
  )
...
export function getNewAssignees(card, newAssignee, oldAssignee) {
  ...
  const assignees = card.issue.assignees.filter(
    assignee =>
          ...
          return repos(card.repoOwner, card.repoName)
            ...
            .then(milestones => {
              ...
              const matchingMilestone = milestones.find(
                _milestone => _milestone.title === milestone.title
              )
